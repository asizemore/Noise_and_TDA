...
#### OPTIONAL filtering
graph_files = filter(x -> !occursin("clique",x), graph_files)
##########
...
for (i,graph_file) in enumerate(graph_files)
    ...
    for threshold in THRESHVEC
        ...
        weighted_graph_array_noise = zeros(NNODES, NNODES, nReps)   # For storing final thresholded with noise matrices
        noise_only_array = zeros(NNODES, NNODES, nReps)    # Storing only the noise part of the threhsolded matrix
        noise_array = zeros(NNODES, NNODES, nReps)
        randomized_noise_array = zeros(NNODES, NNODES, nReps)
        randomized_graph_array = zeros(NNODES, NNODES, nReps)
        ...
        for rep in 1:nReps
            ...
            G_randomized = randomize_edge_weights(G_thresholded)
            ...
            G_randomized[G_randomized .> 0] .= G_randomized[G_randomized .> 0] .+ 1
            ...
            G_randomized_noise = deepcopy(G_randomized)
            G_randomized_noise[G_randomized_noise .== 0] .= noisyGraph[G_randomized_noise .== 0]
            ...
            noise_array[:,:,rep] = noisyGraph
            randomized_noise_array[:,:, rep] = G_randomized_noise
            randomized_graph_array[:,:, rep] = G_randomized
        ...
        save("$(save_dir)/$(save_name)_thresh$(replace(string(threshold), "." => ""))_edge$(threshold_edge_number)_randomized.jld",
            "weighted_graph_array_noise", randomized_noise_array,
            "weighted_graph_randomized", randomized_graph_array,
            "noise_array", noise_array)
