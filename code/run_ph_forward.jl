...
for (i,graph_file) in enumerate(graph_files)
    ...
    if SAVEDATA == 1
        save("./processed_data/results/$(NNODES)nodes/$(replace(graph_file, ".jld" => ""))_eireneoutput_forward.jld",
            "barcodeArray", barcodeArray)
    end

    printstyled("Completed saving eirene outputs for $(graph_models[i]).\n", color = :green)
    println("Saved outputs to ./processed_data/results/$(NNODES)nodes/$(replace(graph_file, ".jld" => ""))_eireneoutput_forward.jld")
    printstyled("Elapsed time = $(time() - script_start_time) seconds \n \n", color = :yellow)
