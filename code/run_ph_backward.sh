...
echo $graph
echo $configfile
/usr/bin/singularity exec noise-and-tda-latest.sif julia --color\=yes --track-allocation\=user code/run_ph_backward.jl "$configfile" $graph
